...
install_bench() {
	run_cmd sudo su $FRAPPE_USER -c "cd /home/$FRAPPE_USER && git clone git@gitlab.com:modernmachines/bench-mirror.git --branch $BENCH_BRANCH bench-repo"
...
setup_bench() {
	...
	echo "From flomente96 github worth 250MB of files. This might take a while..."
	FRAPPE_BRANCH="develop"
	ERPNEXT_APPS_JSON="https://raw.githubusercontent.com/jessacabigas/bench-mirror/master/install_scripts/erpnext-apps.json"
	if $SETUP_PROD; then
		FRAPPE_BRANCH="master"
		ERPNEXT_APPS_JSON="https://raw.githubusercontent.com/jessacabigas/bench-mirror/master/install_scripts/erpnext-apps-master.json"
	fi
